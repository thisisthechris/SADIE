...
2. Render detects `render.yaml` and lists the services it will create. You *should* be prompted for a handful of secret values (`sync: false`, only ever defined directly on `sadie-web`, never inside `envVarGroups` — Render silently ignores `sync: false` on group-level vars and never prompts for them): `MAILGUN_API_KEY`, `MAILGUN_SENDER_DOMAIN`, `DEFAULT_FROM_EMAIL`, `DJANGO_SUPERUSER_USERNAME`/`_EMAIL`/`_PASSWORD`, and optionally `MAPTILER_API_KEY`. **If you don't get prompted** (e.g. because the services already exist from an earlier sync, or the Blueprint was created via a plain `git push` rather than the *New → Blueprint* wizard), add these 7 keys manually on `sadie-web → Environment` in the Dashboard — Render only prompts for `sync: false` values during a brand-new Blueprint's *first* creation flow, never on later syncs.
3. Click *Deploy Blueprint*. The web service's `initialDeployHook` automatically creates the superuser and runs `render_seed` (which waits for the Celery broker to come up, then imports + geocodes the partner CSV test data) on its first successful deploy — no manual shell/CLI access needed. `enable_postgis` (in `preDeployCommand`) similarly retries for a while if the database isn't accepting connections yet on the very first deploy attempt. Both commands are wrapped in `sh -c "..."` since Render execs these commands directly rather than through a shell (so `&&`/`;`/`||` wouldn't otherwise be interpreted).
4. If the very first deploy still fails (e.g. the database took longer than the retry window to come up, or a secret env var was still missing), fix the cause and trigger *Manual Deploy → Deploy latest commit* on `sadie-web` from the Dashboard — `enable_postgis`/`migrate` and the seed step are all safe to re-run.
